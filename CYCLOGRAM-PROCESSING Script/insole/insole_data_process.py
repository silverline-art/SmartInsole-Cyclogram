...
Purpose:
    Convert all CSV files from INSOLE-DATA directory to standardized 87-column format
    matching the reference format (10MWT.csv - Version 23).

Input:
    Source: /home/shivam/Desktop/Human_Pose/PROJECT CYCLOGRAM/INSOLE-DATA/*.csv

Output:
    Target: /home/shivam/Desktop/Human_Pose/PROJECT CYCLOGRAM/insole-sample/Temp-csv/

Format Conversion:
    - Version 207 (5-line header, 39 cols) → Version 23 (3-line header, 87 cols)
    - Version 22/23 (3-line header, 39/63 cols) → Version 23 (3-line header, 87 cols)
...
Date: 2025-10-21
Version: 2.0 - Multi-format column standardization
"""
...
class InsoleCSVStandardizer:
    ...
    def detect_format(self, csv_path: Path) -> Tuple[Optional[int], Optional[List[str]]]:
        ...
        try:
            ...
            print(f"  ⚠ Error detecting format: {csv_path.name} - {e}")
    ...
    def standardize_row(self, source_row: List[str], column_mapping: Dict[str, int]) -> List[str]:
        ...
        for ref_col in self.REFERENCE_COLUMNS:
            ...
            if source_idx >= 0 and source_idx < len(source_row):
                ...
                standardized_row.append(source_row[source_idx])
            else:
    ...
    def convert_to_87_columns(self, source_path: Path, target_path: Path,
        ...
        try:
            ...
            print(f"    Format: {format_type} → 87-col")
            ...
            print(f"  ✗ Conversion failed: {source_path.name} - {e}")
            return False
    ...
    def standardize_file(self, source_path: Path) -> bool:
        ...
        if version is None or headers is None:
            print(f"  ⚠ Could not detect format: {source_path.name}")
            self.skipped += 1
            return False
        ...
        if num_cols == 39 or not has_gyro:
            print(f"  ⚠ SKIPPING (missing GYRO sensors): {source_path.name}")
        ...
        print(f"  🔄 Converting Version {version}: {source_path.name}")
    ...
    def standardize_all(self) -> Tuple[int, int, int]:
        ...
        print("✅ All files standardized to 87-column Version 23 format")
...
def main():
    ...
    SOURCE_DIR = PROJECT_ROOT / "DEMC-DATA"
    TARGET_DIR = PROJECT_ROOT / "insole-sample" / "Temp-csv"
    ...
    # Exit code
    if failed > 0:
        print(f"\n⚠ Exiting with {failed} failures")
        sys.exit(1)
    elif processed == 0:
        print("\n⚠ No files processed")
        sys.exit(1)
    else:
        print("\n✅ All files standardized successfully to 87-column format")
        sys.exit(0)
