...
# PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP PPPPPPPPPPPPPPPPPPPPPP
# IMPORTS
# PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP
...
class AnalysisConfig:
    ...
    # Visualization (DEPRECATED: plot_dpi no longer used, PlotConfig handles DPI)
    plot_dpi: int = 300  # Deprecated - kept for backward compatibility only
...
class PlotConfig:
    ...
    Now supports adaptive sizing based on content and flexible DPI presets.
    """
    ...
    # Base sizes for adaptive scaling (reference sizes for standard content)
    base_cyclogram_figsize: Tuple[float, float] = (8, 6)  # Single subplot reference
    base_similarity_figsize: Tuple[float, float] = (10, 6)

    # Size constraints (min/max bounds for adaptive scaling)
    ...
    # Resolution (direct override, or use dpi_preset)
    dpi: int = 150  # Default to screen quality (was 300)
    ...
    tick_max_bins_cyclogram: int = 6  # Adaptive tick bins for cyclogram axes (based on actual data range)
    tick_max_bins_similarity: int = 8  # Maximum tick bins for similarity plots
    ...
    def calibrate_quality_thresholds(self, quality_scores: List[float]) -> None:
        """
        Calibrate quality thresholds based on data distribution using MAD.
        ...
        # Use MAD-based thresholds (median ± k*MAD)
        # Typically: k=1.4826 to match std dev for normal distribution
        scale_factor = 1.4826

        # Excellent: median + 0.5*MAD*scale
        # Good: median
        # Acceptable: median - 0.5*MAD*scale
        self.excellent_threshold = min(100, median_score + 0.5 * mad * scale_factor)
        self.good_threshold = median_score
        self.acceptable_threshold = max(0, median_score - 0.5 * mad * scale_factor)

        print(f"  ⚙️  Calibrated quality thresholds (MAD-based): "
    ...
    def get_dpi(self) -> int:
        ...
        Presets:
        - 'web': 96 DPI (web graphics, smallest files)
        - 'screen': 150 DPI (screen viewing, balanced quality/size)
        ...
        # Use preset if available, otherwise use direct dpi value
    ...
    def calculate_cyclogram_figsize(self, num_cycles: int = 5) -> Tuple[float, float]:
        ...
        Args:
            num_cycles: Number of gait cycles being plotted (affects density in adaptive mode)
        ...
        # Side-by-side layout (L + R subplots) - scale width for two panels
        width = base_w * 2.0

        # Slight height increase for more cycles (improves readability)
        # Uses logarithmic scaling to avoid excessive growth
    ...
    def calculate_similarity_figsize(self, num_metrics: int = 6) -> Tuple[float, float]:
        """
        Calculate adaptive figure size for similarity summary plots.
        ...
        Args:
            num_metrics: Number of metrics being displayed
        ...
        if self.sizing_mode == 'fixed':
            return self.similarity_figsize

        # Adaptive sizing based on content
        base_w, base_h = self.base_similarity_figsize

        # Scale width slightly for more metrics
        width_factor = min(1.3, 1.0 + num_metrics / 20)
...
def standard_plot_setup(plot_cfg: 'PlotConfig',
    ...
    """
    Create a standardized figure for consistent look across all plots.

    This utility function ensures uniform plot dimensions, DPI, fonts, and styling
    across all visualizations, particularly useful for publication-grade outputs.

    Args:
        plot_cfg: PlotConfig instance with plot settings
        title: Figure title
        ...
        figsize: Override figure size (width, height) in inches.
                If None, uses plot_cfg settings (default: None)

    Returns:
        (fig, axes): Matplotlib Figure and axes (single Axes or ndarray)

    Example:
        >>> plot_cfg = PlotConfig(sizing_mode='publication')
        >>> fig, axes = standard_plot_setup(plot_cfg, "Gait Cyclograms", nrows=2, ncols=3)
        >>> # axes is now a 2x3 array with uniform styling
    """
    import matplotlib.pyplot as plt

    # Determine figure size
    if figsize is None:
        if plot_cfg.sizing_mode == 'publication':
            figsize = plot_cfg.publication_figsize
        else:
            # Use cyclogram size as default for multi-panel layouts
            figsize = plot_cfg.calculate_cyclogram_figsize()

    # Get DPI
    dpi = plot_cfg.get_dpi()
    ...
    # Apply title with configured styling
    fig.suptitle(title,
                fontsize=plot_cfg.title_fontsize,
                fontweight=plot_cfg.title_fontweight)

    # Apply tight layout with padding
    plt.tight_layout(pad=1.5)

    # Configure all axes if multiple subplots
    if nrows * ncols > 1:
        ...
        for ax in axes_flat:
            ...
            ax.tick_params(labelsize=plot_cfg.tick_fontsize)
        ...
        if hasattr(axes, 'grid'):
            axes.grid(True,
                     linestyle=plot_cfg.grid_linestyle,
                     alpha=plot_cfg.grid_alpha)
            axes.tick_params(labelsize=plot_cfg.tick_fontsize)
...
class DataQualityMetrics:
    """
    Comprehensive data quality assessment with robust statistics.
    ...
    sampling_rate_median: float  # Robust center
    sampling_rate_mad: float  # Robust spread (MAD)
    temporal_completeness: float

    # Signal quality
    snr_estimates: Dict[str, float]
    jump_severity_max: float
    jump_severity_median: float  # Robust measure
    jump_count: int

    # Angle variability (using MAD)
    angle_variability_mad: Dict[str, float]  # Per-angle MAD
    ...
    def compute(angles_df: pd.DataFrame,
        ...
        for leg, joints in col_map.items():
            ...
            per_leg_coverage[leg] = np.median(leg_coverage)  # Use median for robustness
        ...
        median_gap = np.median(gap_sizes) if gap_sizes else 0

        # 3. Temporal consistency using MAD
        if 'timestamp' in angles_df.columns:
            time_diffs = angles_df['timestamp'].diff().dropna()
            sampling_rate_median = time_diffs.median()
            sampling_rate_mad = DataQualityMetrics.compute_mad(time_diffs.values)

            expected_frames = (angles_df['timestamp'].max() - angles_df['timestamp'].min()) / sampling_rate_median
            temporal_completeness = (len(angles_df) / expected_frames) * 100 if expected_frames > 0 else 100
        else:
            sampling_rate_median = 0.0
            sampling_rate_mad = 0.0
            temporal_completeness = 100.0

        # 4. Signal quality (SNR estimate)
        snr_estimates = {}
        for col in angle_cols:
            valid_data = angles_df[col].dropna()
            if len(valid_data) > 10:
                signal_var = valid_data.var()
                noise_var = valid_data.diff().dropna().var() / 2
                snr = 10 * np.log10(signal_var / noise_var) if noise_var > 0 else 50.0
                snr_estimates[col] = max(0, min(50, snr))
            else:
                snr_estimates[col] = 0.0

        # 5. Jump detection with robust statistics
        jump_severity_max = 0
        jump_count_total = 0
        all_jumps = []
        for col in angle_cols:
            ...
            if not jumps.empty:
                ...
                all_jumps.extend(jumps.values)

        jump_severity_median = np.median(all_jumps) if all_jumps else 0.0

        # 6. Angle variability using MAD
        angle_variability_mad = {}
        for col in angle_cols:
            valid_data = angles_df[col].dropna()
            if len(valid_data) > 10:
                mad = DataQualityMetrics.compute_mad(valid_data.values)
                angle_variability_mad[col] = mad
            else:
                angle_variability_mad[col] = 0.0

        # 7. Composite quality score using robust metrics
        coverage_score = overall_coverage
        gap_score = max(0, 100 - (max_gap / config.max_gap_frames) * 100)
        snr_score = np.median(list(snr_estimates.values())) * 2
        ...
        return DataQualityMetrics(
            ...
            sampling_rate_mad=sampling_rate_mad,
            temporal_completeness=temporal_completeness,
            snr_estimates=snr_estimates,
            jump_severity_max=jump_severity_max,
            jump_severity_median=jump_severity_median,
            jump_count=jump_count_total,
            angle_variability_mad=angle_variability_mad,
            quality_score=quality_score
        )

    def print_report(self) -> None:
        """Print formatted quality report with robust statistics."""
        print(f"\n{'='*80}")
        print(f"📊 DATA QUALITY REPORT (Robust Statistics)")
        print(f"{'='*80}")
        ...
        for leg, cov in self.per_leg_coverage.items():
            print(f"  {leg} leg (median): {cov:.1f}%")
        print(f"\n🔍 Gaps:")
        print(f"  Max gap: {self.max_gap_frames} frames")
        print(f"  Median gap: {self.median_gap_size:.1f} frames (robust)")
        print(f"  Mean gap: {self.mean_gap_size:.1f} frames")
        print(f"  Total gaps: {self.gap_count}")
        print(f"\n📡 Signal Quality (SNR):")
        for joint, snr in list(self.snr_estimates.items())[:6]:  # Show first 6
            print(f"  {joint}: {snr:.1f} dB")
        print(f"\n⏱️  Temporal Consistency (MAD-based):")
        print(f"  Sampling rate (median): {self.sampling_rate_median:.4f}s")
        print(f"  Sampling rate MAD: {self.sampling_rate_mad:.4f}s")
        print(f"  Completeness: {self.temporal_completeness:.1f}%")
        print(f"\n⚡ Angle Jumps:")
        print(f"  Max severity: {self.jump_severity_max:.1f}°")
        print(f"  Median severity: {self.jump_severity_median:.1f}° (robust)")
        print(f"  Total count: {self.jump_count}")
        print(f"\n📏 Angle Variability (MAD):")
        for joint, mad in list(self.angle_variability_mad.items())[:6]:
            print(f"  {joint}: MAD={mad:.2f}°")
        print(f"{'='*80}\n")
...
def calibrate_filter_parameters(angles_df: pd.DataFrame,
    ...
    """
    Calibrate Savitzky-Golay filter parameters based on signal characteristics using MAD.

    Uses FFT analysis to determine dominant frequencies and MAD for robust
    variability estimation.

    Returns:
        (window_length, poly_order)
    """
    # Analyze signal frequency content and variability
    signal_frequencies = []
    signal_mads = []
    ...
    if signal_frequencies:
        ...
        # Fallback based on MAD
        if signal_mads:
            median_mad = np.median(signal_mads)
            # Higher variability → larger window for smoothing
            if median_mad > 15:
                window_length = 15
            elif median_mad > 10:
                window_length = 11
            ...
            window_length = 11  # Default

    # Polynomial order: lower for noisy data (high MAD)
    if signal_mads:
        median_mad = np.median(signal_mads)
        if median_mad > 15 or window_length < 11:
            poly_order = 2  # Lower order for noisy/short windows
        else:
            poly_order = 3  # Higher order for clean data
    else:
        poly_order = 2

    print(f"  ⚙️  Calibrated filter (FFT + MAD): window={window_length}, poly_order={poly_order}")
    if signal_frequencies:
        print(f"     Dominant frequency (median): {np.median(signal_frequencies):.2f}Hz")
    if signal_mads:
        print(f"     Signal variability (MAD median): {np.median(signal_mads):.2f}°")
...
def auto_calibrate_config(angles_df: pd.DataFrame,
    ...
    """
    Enhanced automatic calibration with comprehensive parameter optimization.

    Calibrates:
    1. Data quality assessment (NEW - using MAD)
    2. Filter parameters (NEW - FFT + MAD based)
    ...
    Returns:
        Tuple of (calibrated_config, quality_metrics)
    """
    print(f"\n{'─'*80}")
    ...
    # 1. Compute comprehensive data quality metrics (NEW)
    quality_metrics = DataQualityMetrics.compute(angles_df, col_map, base_config)
    quality_metrics.print_report()

    # 2. Calibrate filter parameters using FFT + MAD (NEW)
    smooth_window, smooth_poly = calibrate_filter_parameters(angles_df, angle_cols, base_config.fps)

    # 3. Calibrate quality gate thresholds based on data quality (NEW)
    if quality_metrics.quality_score < 50:
        # Relaxed thresholds for poor-quality data
        min_coverage_pct = 50.0
        max_gap_frames = 50
        print(f"  ⚙️  Relaxed quality gates: coverage≥{min_coverage_pct}%, gaps≤{max_gap_frames}f (low data quality: {quality_metrics.quality_score:.1f})")
    elif quality_metrics.quality_score < 75:
        min_coverage_pct = 60.0
        max_gap_frames = 40
        print(f"  ⚙️  Moderate quality gates: coverage≥{min_coverage_pct}%, gaps≤{max_gap_frames}f")
    else:
        min_coverage_pct = 70.0
        max_gap_frames = 30
        print(f"  ⚙️  Standard quality gates: coverage≥{min_coverage_pct}%, gaps≤{max_gap_frames}f")

    # 4. Calibrate smoothing threshold using MAD (enhanced)
    smooth_threshold_values = []
    for col in angle_cols:
        valid_data = angles_df[col].dropna()
        if len(valid_data) > 10:
            # Use MAD for robust threshold
            mad = DataQualityMetrics.compute_mad(valid_data.values)
            # Threshold = 2.5 * MAD (equivalent to ~1.7 std for normal distribution)
            threshold = 2.5 * mad * 1.4826
            smooth_threshold_values.append(threshold)

    smooth_threshold = np.median(smooth_threshold_values) if smooth_threshold_values else 15.0
    print(f"  ⚙️  Calibrated smoothing threshold (MAD-based): {smooth_threshold:.2f}°")

    # 5. Calibrate CYCLE duration constraints (HS→HS) for each leg
    ...
    # 6. Extract stride durations for tolerance calibration
    ...
    # Create enhanced config with all calibrated parameters
    calibrated_config = AnalysisConfig(
        fps=base_config.fps,
        smooth_window=smooth_window,  # CALIBRATED (FFT + MAD)
        smooth_poly=smooth_poly,  # CALIBRATED (FFT + MAD)
        smooth_threshold=smooth_threshold,  # CALIBRATED (MAD-based)
        min_coverage_pct=min_coverage_pct,  # CALIBRATED (adaptive)
        max_gap_frames=max_gap_frames,  # CALIBRATED (adaptive)
        ...
        max_angle_jump=base_config.max_angle_jump,
        max_pelvis_std=base_config.max_pelvis_std,
        use_pchip=base_config.use_pchip,
        pchip_confidence=base_config.pchip_confidence,
        use_index_pairing=base_config.use_index_pairing,
        min_phase_overlap=base_config.min_phase_overlap
    )
...
def build_cycle_windows(events_df: pd.DataFrame,
    ...
    # Enhanced error messaging when ALL cycles are rejected
    if apply_quality_gates and angles_df is not None:
        extracted_count = rejection_counts['total_extracted']
        rejected_count = extracted_count - len(windows)

        if rejected_count > 0 and rejected_count == extracted_count and extracted_count > 0:
            # ALL cycles rejected - analyze WHY and provide guidance
            print(f"\n{'='*80}")
            print(f"❌ QUALITY GATE FAILURE ({leg} leg): All {extracted_count} cycles rejected")
            print(f"{'='*80}")

            # Analyze data coverage
            coverage_issues = []
            # Build column map for this specific leg
            leg_suffix = f"_{leg}_" if len(leg) == 1 else f"_{leg.lower()}_"
            for joint in ['hip', 'knee', 'ankle']:
                # Find column matching this joint and leg
                col_found = None
                for col in angles_df.columns:
                    col_lower = col.lower()
                    # Must contain joint name AND leg identifier AND degree/angle indicator
                    if (joint in col_lower and
                        leg_suffix in col_lower and
                        ('deg' in col_lower or 'angle' in col_lower)):
                        col_found = col
                        break

                # Check coverage if column found
                if col_found:
                    total_frames = len(angles_df)
                    valid_frames = angles_df[col_found].notna().sum()
                    coverage = (valid_frames / total_frames) * 100 if total_frames > 0 else 0

                    min_coverage = config.min_coverage_pct if config else 70.0
                    if coverage < min_coverage:
                        coverage_issues.append(
                            f"{col_found}: {coverage:.1f}% < {min_coverage:.0f}% threshold")

            # Show all rejection reasons
            reasons_shown = False
            if coverage_issues:
                print(f"Primary issue: Insufficient data coverage")
                for issue in coverage_issues:
                    print(f"  • {issue}")
                reasons_shown = True

            # Show other rejection reasons
            if rejection_counts['rejected_gaps'] > 0:
                if not reasons_shown:
                    print(f"Primary issue:")
                print(f"  • Large NaN gaps (>{config.max_gap_frames if config else 30} frames) in {rejection_counts['rejected_gaps']} cycles")
                reasons_shown = True

            if rejection_counts['rejected_sanity'] > 0:
                if not reasons_shown:
                    print(f"Primary issue:")
                print(f"  • Unrealistic angle jumps (>{config.max_angle_jump if config else 45}°/frame) in {rejection_counts['rejected_sanity']} cycles")
                reasons_shown = True

            if rejection_counts['rejected_stability'] > 0:
                if not reasons_shown:
                    print(f"Primary issue:")
                print(f"  • Unstable pelvis motion in {rejection_counts['rejected_stability']} cycles")
                reasons_shown = True

            if rejection_counts['rejected_coverage'] > 0:
                if not reasons_shown:
                    print(f"Primary issue:")
                print(f"  • Insufficient coverage (<{config.min_coverage_pct if config else 70.0}%) in {rejection_counts['rejected_coverage']} cycles")
                reasons_shown = True

            print(f"\n💡 SOLUTION: Use --enhance-angles flag to fill gaps and recalculate missing data")
            print(f"   python3 Pose-Analysis.py --input-dir <path> --enhance-angles")
            print(f"{'='*80}\n")
...
def predict_cyclogram_from_trends(angles_df: pd.DataFrame,
    ...
    """
    Predict cyclogram using MMC (Morphological Mean Cyclogram) from valid neighboring cyclograms.

    Strategy:
    1. Use MMC median trajectory from valid strides as baseline (robust to outliers)
    2. Apply temporal interpolation based on stride timing
    3. Scale based on stride duration

    Falls back to naive mean if MMC computation fails.
    """
    if not valid_cyclograms or len(valid_cyclograms) < 2:
        return None

    # Try to compute MMC (Morphological Mean Cyclogram)
    mmc = compute_morphological_mean(valid_cyclograms)

    if mmc is not None:
        # Use MMC median trajectory (phase-aligned, robust)
        baseline_proximal = mmc.median_proximal
        baseline_distal = mmc.median_distal
    else:
        # Fallback to naive mean if MMC fails
        baseline_proximal = np.mean([c.proximal for c in valid_cyclograms], axis=0)
        baseline_distal = np.mean([c.distal for c in valid_cyclograms], axis=0)
    ...
    proximal_scaled = baseline_proximal * np.clip(duration_ratio, 0.5, 1.5)
    distal_scaled = baseline_distal * np.clip(duration_ratio, 0.5, 1.5)
...
# PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP
# MORPHOLOGICAL MEAN CYCLOGRAM (MMC) - Phase-Aligned Median Averaging
# PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP
...
class MorphologicalMeanCyclogram:
    ...
    Preserves real gait shape and phase continuity without false smoothing.
    """
    leg: Literal["L", "R"]
    joint_pair: Tuple[str, str]
    ...
    shape_dispersion_index: float  # mean(variance) / median(area) - dimensionless variability
    confidence_ellipse_params: Dict[str, float]  # center, axes, angle for 95% CI
    n_loops: int  # number of loops used in computation
    alignment_quality: float  # mean DTW distance to median reference
    median_area: float  # median loop area (before normalization)

    @property
    def median_proximal(self) -> np.ndarray:
        """Proximal joint angles (101 points)."""
        return self.median_trajectory[:, 0]

    @property
    def median_distal(self) -> np.ndarray:
        """Distal joint angles (101 points)."""
        return self.median_trajectory[:, 1]


def find_median_reference_loop(loops: List[CyclogramLoop]) -> int:
    """
    Find median reference loop using DTW distances.

    Returns index of loop that minimizes sum of DTW distances to all other loops.
    This is the "most representative" loop for phase alignment.
    """
    if len(loops) == 1:
        return 0
    ...
    # Compute sum of DTW distances for each loop
    for i in range(n):
        for j in range(n):
            if i != j:
                dtw_sum[i] += dtw_distance(loops[i], loops[j])

    # Return index with minimum sum
    return int(np.argmin(dtw_sum))


def center_loop(loop: CyclogramLoop) -> Tuple[np.ndarray, np.ndarray]:
    """
    Center loop to centroid (removes position bias).

    Returns (proximal_centered, distal_centered).
    """
    centroid_x = np.nanmean(loop.proximal)
    centroid_y = np.nanmean(loop.distal)

    proximal_centered = loop.proximal - centroid_x
    distal_centered = loop.distal - centroid_y

    return proximal_centered, distal_centered


def rescale_loop(proximal: np.ndarray, distal: np.ndarray,
                 target_area: float, current_area: float) -> Tuple[np.ndarray, np.ndarray]:
    """
    Rescale loop to target area (removes amplitude bias).

    Returns (proximal_scaled, distal_scaled).
    """
    if abs(current_area) < 1e-6:
        # Degenerate loop, return as-is
        return proximal, distal

    scale_factor = np.sqrt(abs(target_area) / abs(current_area))

    proximal_scaled = proximal * scale_factor
    distal_scaled = distal * scale_factor

    return proximal_scaled, distal_scaled


def compute_confidence_ellipse(loops: List[CyclogramLoop]) -> Dict[str, float]:
    """
    Compute 95% confidence ellipse parameters from loop centroids.

    Returns dict with: center_x, center_y, major_axis, minor_axis, angle
    """
    # Extract centroids
    centroids = np.array([[np.nanmean(loop.proximal), np.nanmean(loop.distal)]
                          for loop in loops])
    ...
    # 95% confidence interval (chi-square 2-DOF, factor = 5.991)
    chi2_val = 5.991
...
def compute_morphological_mean(loops: List[CyclogramLoop]) -> Optional[MorphologicalMeanCyclogram]:
    """
    Compute Morphological Mean Cyclogram (MMC) with phase-aligned median averaging.

    Algorithm:
    1. Filter valid loops (sufficient non-NaN coverage)
    2. Find median reference loop (minimize DTW distance sum)
    3. Center all loops to centroid
    4. Rescale all loops to median area
    5. Compute median shape (50th percentile per phase point)
    6. Compute variance envelope (±1 SD per phase point)
    7. Calculate shape dispersion index
    8. Fit confidence ellipse
    ...
    # Filter valid loops (< 20% NaN)
    valid_loops = [loop for loop in loops if loop.nan_percent < 20.0]

    if len(valid_loops) < 2:
        return None

    # Step 1: Find median reference loop
    median_ref_idx = find_median_reference_loop(valid_loops)

    # Step 2: Compute areas for all loops
    areas = [calculate_loop_area(loop) for loop in valid_loops]
    median_area = float(np.median(np.abs(areas)))

    # Step 3: Center and rescale all loops
    centered_rescaled_loops = []
    for loop, area in zip(valid_loops, areas):
        # Center to centroid
        prox_centered, dist_centered = center_loop(loop)

        # Rescale to median area
        prox_scaled, dist_scaled = rescale_loop(prox_centered, dist_centered,
                                                 median_area, area)

        centered_rescaled_loops.append(np.column_stack([prox_scaled, dist_scaled]))

    # Step 4: Stack loops for vectorized computation
    loops_array = np.stack(centered_rescaled_loops, axis=0)  # Shape: (N, 101, 2)

    # Step 5: Compute median trajectory (50th percentile per phase point)
    median_trajectory = np.nanmedian(loops_array, axis=0)  # Shape: (101, 2)

    # Step 6: Compute variance envelope (±1 SD per phase point)
    std_trajectory = np.nanstd(loops_array, axis=0)  # Shape: (101, 2)
    envelope_lower = median_trajectory - std_trajectory
    envelope_upper = median_trajectory + std_trajectory

    # Step 7: Calculate shape dispersion index
    var_proximal = np.nanvar(loops_array[:, :, 0], axis=0)  # Shape: (101,)
    var_distal = np.nanvar(loops_array[:, :, 1], axis=0)    # Shape: (101,)
    mean_variance = float(np.mean(var_proximal) + np.mean(var_distal))
    shape_dispersion_index = mean_variance / max(median_area, 1e-6)

    # Step 8: Compute alignment quality (mean DTW to median reference)
    dtw_distances = [dtw_distance(valid_loops[i], valid_loops[median_ref_idx])
                     for i in range(len(valid_loops))]
    alignment_quality = float(np.mean(dtw_distances))

    # Step 9: Fit confidence ellipse
    confidence_ellipse = compute_confidence_ellipse(valid_loops)

    return MorphologicalMeanCyclogram(
        leg=valid_loops[0].leg,
        joint_pair=valid_loops[0].joint_pair,
...
def plot_overlayed_cyclograms(loops_L: List[CyclogramLoop],
    ...
    # Calculate adaptive figure size based on number of cycles
    ...
    # Calculate and plot MMC (Morphological Mean Cyclogram) for left loop
    if loops_L:
        mmc_L = compute_morphological_mean(loops_L)

        if mmc_L is not None:
            # Plot variance envelope (±1 SD shaded region)
            lower = mmc_L.variance_envelope_lower
            upper = mmc_L.variance_envelope_upper
            ax_left.fill(np.concatenate([lower[:, 0], upper[::-1, 0]]),
                        np.concatenate([lower[:, 1], upper[::-1, 1]]),
                        alpha=0.15, color=plot_config.left_color, label='±1 SD', zorder=5)

            # Plot median trajectory (MMC)
            median_traj = mmc_L.median_trajectory
            ax_left.plot(median_traj[:, 0], median_traj[:, 1],
                        color=plot_config.left_mean_color, linewidth=plot_config.mean_linewidth,
                        label='Median (MMC)', zorder=10,
                        marker=plot_config.mean_marker, markevery=plot_config.mean_markevery,
                        markersize=plot_config.mean_markersize)

            # Add MMC label at midpoint (only if not NaN)
            mid_idx = len(median_traj) // 2
            if not (np.isnan(median_traj[mid_idx, 0]) or np.isnan(median_traj[mid_idx, 1])):
                ax_left.text(median_traj[mid_idx, 0], median_traj[mid_idx, 1], 'MMC',
            ...
            # Fallback to simple mean if MMC fails
    ...
    if loops_L:
        ...
        if area_values:
            ...
            # Add MMC shape dispersion index if available
            if 'mmc_L' in locals() and mmc_L is not None:
                stats_left += f"SDI: {mmc_L.shape_dispersion_index:.3f}\n"
    ...
    # Apply tick locators for clean spacing (adaptive to actual data range)
    ax_left.xaxis.set_major_locator(MaxNLocator(nbins=plot_config.tick_max_bins_cyclogram, integer=False, prune='both'))
    ax_left.yaxis.set_major_locator(MaxNLocator(nbins=plot_config.tick_max_bins_cyclogram, integer=False, prune='both'))
    ax_left.tick_params(axis='both', which='major', labelsize=plot_config.tick_fontsize)
    ...
    # Calculate and plot MMC (Morphological Mean Cyclogram) for right loop
    if loops_R:
        mmc_R = compute_morphological_mean(loops_R)

        if mmc_R is not None:
            # Plot variance envelope (±1 SD shaded region)
            lower = mmc_R.variance_envelope_lower
            upper = mmc_R.variance_envelope_upper
            ax_right.fill(np.concatenate([lower[:, 0], upper[::-1, 0]]),
                         np.concatenate([lower[:, 1], upper[::-1, 1]]),
                         alpha=0.15, color=plot_config.right_color, label='±1 SD', zorder=5)

            # Plot median trajectory (MMC)
            median_traj = mmc_R.median_trajectory
            ax_right.plot(median_traj[:, 0], median_traj[:, 1],
                         color=plot_config.right_mean_color, linewidth=plot_config.mean_linewidth,
                         label='Median (MMC)', zorder=10,
                         marker=plot_config.mean_marker, markevery=plot_config.mean_markevery,
                         markersize=plot_config.mean_markersize)
            ...
            mid_idx = len(median_traj) // 2
            if not (np.isnan(median_traj[mid_idx, 0]) or np.isnan(median_traj[mid_idx, 1])):
                ax_right.text(median_traj[mid_idx, 0], median_traj[mid_idx, 1], 'MMC',
    ...
    if loops_R:
        ...
        if area_values:
            ...
            # Add MMC shape dispersion index if available
            if 'mmc_R' in locals() and mmc_R is not None:
                stats_right += f"SDI: {mmc_R.shape_dispersion_index:.3f}\n"
    ...
    # Apply tick locators for clean spacing (adaptive to actual data range)
    ax_right.xaxis.set_major_locator(MaxNLocator(nbins=plot_config.tick_max_bins_cyclogram, integer=False, prune='both'))
    ax_right.yaxis.set_major_locator(MaxNLocator(nbins=plot_config.tick_max_bins_cyclogram, integer=False, prune='both'))
    ax_right.tick_params(axis='both', which='major', labelsize=plot_config.tick_fontsize)
    ...
    plt.savefig(output_path,
               dpi=dpi,  # Use calculated DPI from get_dpi()
...
def plot_similarity_summary(session_summary: pd.DataFrame,
    ...
    # Calculate adaptive figure size based on number of metrics
    ...
    # Apply tick locator for clean y-axis spacing
    ax.yaxis.set_major_locator(MaxNLocator(nbins=plot_config.tick_max_bins_similarity, integer=False))
...
def analyze_subject(subject_dir: Path,
    ...
    try:
        ...
        # AUTO-ENHANCEMENT: Check data quality and auto-enable enhancement if needed
        if config.auto_enhance and not config.enhance_angles:
            # Quick quality check before processing
            try:
                col_map_temp = detect_columns(angles_df)
                angle_cols_temp = [col for leg in col_map_temp.values() for col in leg.values()]

                # Calculate overall coverage
                total_values = len(angles_df) * len(angle_cols_temp)
                valid_values = angles_df[angle_cols_temp].notna().sum().sum()
                overall_coverage = (valid_values / total_values) * 100 if total_values > 0 else 0

                # Auto-enable enhancement if coverage is below threshold
                if overall_coverage < config.auto_enhance_threshold:
                    print(f"\n{'─'*80}")
                    print(f"⚠️  LOW DATA QUALITY DETECTED")
                    print(f"{'─'*80}")
                    print(f"   Coverage: {overall_coverage:.1f}% (threshold: {config.auto_enhance_threshold:.0f}%)")
                    print(f"   Auto-enabling angle enhancement to improve analysis quality...")
                    print(f"   (Disable with --no-auto-enhance flag)")
                    print(f"{'─'*80}\n")
                    config.enhance_angles = True
            except Exception as e:
                # If quick check fails, proceed without auto-enhancement
                pass

        # Enhancement: Recalculate missing angles from keypoints if requested or auto-enabled
    ...
    try:
        config, quality_metrics = auto_calibrate_config(angles_df, events_df, col_map, config, min_cycles=5)
    except Exception as e:
    ...
    try:
        for joint_pair in config.joint_pairs:
            ...
            # Use provided plot configuration (from CLI or default)
...
def run_batch_analysis(input_dir: str, output_dir: str, config: AnalysisConfig,
                      plot_config: Optional['PlotConfig'] = None) -> None:
    """
    Run analysis on all subjects in input directory.

    Args:
        input_dir: Path to input directory containing subject folders
        output_dir: Path to output directory
        config: Analysis configuration
        plot_config: Optional plot configuration for standardized visualizations
    ...
    for i, subject_dir in enumerate(subject_dirs, 1):
        ...
        try:
            result = analyze_subject(subject_dir, output_path, config, plot_config)
            if result:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Run analysis on all subjects (publication mode by default: 10×8" @ 300 DPI)
  python3 Analysis.py

  # Run on specific subject (publication mode by default)
  python3 Analysis.py --subject-name "Openpose_subject_001"

  # Run with adaptive mode for content-aware sizing
  python3 Analysis.py --adaptive-mode

  # Run with custom parameters
  python3 Analysis.py --smooth-window 15 --lock-axis-limits

  # Disable angle enhancement if needed
  python3 Analysis.py --no-enhance-angles
        """
    )

    parser.add_argument('--subject-name', type=str, default=None,
                       help='Specific subject to analyze (default: all subjects)')
    parser.add_argument('--no-enhance-angles', action='store_true',
                       help='Disable keypoint-based angle recalculation (enabled by default)')
    parser.add_argument('--no-auto-enhance', action='store_true',
                       help='Disable automatic enhancement for poor quality data')
    ...
    parser.add_argument('--lock-axis-limits', action='store_true',
                       help='Lock axis limits for consistent scaling across all plots (enabled in publication mode)')
    ...
    config = AnalysisConfig(
        ...
        enhance_angles=not args.no_enhance_angles,  # Enable by default, disable with --no-enhance-angles
        auto_enhance=not args.no_auto_enhance  # Enable by default, disable with flag
    )

    # Create global plot configuration
    # Publication mode is default for uniform, professional outputs
    ...
    if args.subject_name:
        ...
        if not subject_dir.exists():
            ...
            analyze_subject(subject_dir, output_path, config, GLOBAL_PLOT_CONFIG)
    else:
        # Run batch analysis
        run_batch_analysis(input_path, output_path, config, GLOBAL_PLOT_CONFIG)
